...
class Movie {
	...
	public function custom_post_type_css() {
		...
		if ( ( $pagenow !== 'post.php' ) && $typenow !== 'movie' ) {
			return;
	    }
	...
	public function register_post_type() {
		$labels = array(
			'name'                  => _x( 'Films', 'Film Nom pluriel', $this->theme_name ),
	        'singular_name'         => _x( 'Film', 'Film Nom singulier', $this->theme_name ),
	        'menu_name'             => __( 'Films', $this->theme_name ),
	        'name_admin_bar'        => __( 'Film', $this->theme_name ),
	        'parent_item_colon'     => __( '', $this->theme_name ),
	        'all_items'             => __( 'Tous les films', $this->theme_name ),
	        'add_new_item'          => __( 'Ajouter un nouveau film', $this->theme_name ),
	        'add_new'               => __( 'Ajouter', $this->theme_name ),
	        'new_item'              => __( 'Nouveau film', $this->theme_name ),
	        'edit_item'             => __( 'Modifier le film', $this->theme_name ),
	        'update_item'           => __( 'Mettre à jour le film', $this->theme_name ),
	        'view_item'             => __( 'Voir le film', $this->theme_name ),
	        'view_items'            => __( 'Voir les films', $this->theme_name ),
	        'search_items'          => __( 'Chercher parmi les films', $this->theme_name ),
	        'not_found'             => __( 'Aucun film trouvé.', $this->theme_name ),
	        'not_found_in_trash'    => __( 'Aucun film trouvé dans la corbeille.', $this->theme_name ),
	        ...
	        'insert_into_item'      => __( 'Insérer dans le film', $this->theme_name ),
	        'uploaded_to_this_item' => __( 'Ajouter à ce film', $this->theme_name ),
	        'items_list'            => __( 'Liste des films', $this->theme_name ),
	        'items_list_navigation' => __( 'Navigation de liste des films', $this->theme_name ),
	        'filter_items_list'     => __( 'Filtrer la liste des films', $this->theme_name ),
		);
		$rewrite = array(
	        'slug'                	=> 'films',
	        'with_front'          	=> true,
		...
		$args = array(
			'label'               	=> 'film',
	        'description'         	=> __( 'Les films', $this->theme_name ),
	        'labels'              	=> $labels,
        ...
        switch ( $column_name ) {
    		...
    		case 'release_year' :
    			...
    			if ( get_field( 'release_year', $post_id ) ) {
    				the_field( 'release_year', $post_id );
    			} else {
    				echo '—';
    			}
	        break;
    		...
    		case 'running_time' :
    			...
    			if ( get_field( 'running_time', $post_id ) ) {
    				the_field( 'running_time', $post_id );
    			} else {
    				echo '—';
    			}
	        break;
